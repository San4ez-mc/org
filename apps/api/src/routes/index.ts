...
api.post('/companies/:id/propose-structure', async (req, res) => {
  try {
    ...
    const prompt = `Ти — консультант з орг-структури. За канонічним шаблоном і НАЯВНИМИ теками компанії "${c.name}"${c.abbr ? ` (префікс ${c.abbr})` : ''} запропонуй впорядковану структуру папок Google Drive.\n\nГОЛОВНІ ПРИНЦИПИ:\n1. Реструктуризуй МІНІМАЛЬНО. Бери наявні теки за ОСНОВУ, зберігай їхні назви де доречно, не вигадуй зайвих сутностей. Мета — впорядкувати, а не переносити все.\n2. Структура має відображати РЕАЛЬНУ орг-структуру компанії (реальні відділи/посади видно з наявних тек) — а не абстрактний шаблон. Шаблон — орієнтир, не догма.\n3. КОЖНА тека ОБОВ'ЯЗКОВО має descUser і descSystem (це критично!):\n   - descUser: 1 стисле речення — що людина тут зберігає.\n   - descSystem: 3-7 ключових слів/типів документів — за ними система АВТОМАТИЧНО розкладатиме майбутні файли в правильну теку. Будь конкретним.\n4. Для КОЖНОЇ теки вкажи ПЛАН ЗМІН відносно наявних тек:\n   - action: "keep" (наявна тека без змін), "rename" (наявна, змінюється лише назва), "move" (наявна тека переноситься в інше місце), "new" (нова тека, якої не було).\n   - origin: назва/шлях наявної теки, з якої ця походить (для keep/rename/move); "" для new.\n\nСТИСЛІСТЬ (щоб уміститись): показуй структуру ДО РІВНЯ ПОСАД. НЕ розгортай стандартні підпапки посади (Посадові інструкції/Доступні документи/Доступи/Звітність) — згадай їх ОДИН раз у descUser відділення.\n\nПоверни РІВНО один JSON без тексту навколо:\n{"structure":[{"name":"...","type":"folder","descUser":"...","descSystem":"...","action":"keep|rename|move|new","origin":"","children":[...]}]}\nУкраїнською.\n\nШАБЛОН-ОРІЄНТИР:\n${STRUCTURE_TEMPLATE}\n\nНАЯВНІ ТЕКИ (основа):\n${current}`;

    const text = await flowsGenerate(prompt, 24576);
    let parsed: any = null;
    if (text) {
