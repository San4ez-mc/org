...
api.get('/companies/:id/dashboard', async (req, res) => {
  try {
    ...
    // Вузькі місця — непорожні прогалини, від найкритичніших.
    ...
    const dashboard = {
      staffing: {
        ...
        filled,
        vacant,
        filledPct: postsTotal > 0 ? Math.round((filled / postsTotal) * 100) : 0,
      },
      processes: {
        total: processesTotal,
        described: processesDescribed,
        describedPct: processesTotal > 0 ? Math.round((processesDescribed / processesTotal) * 100) : 0,
      },
