...
api.post('/companies/:id/propose-structure', async (req, res) => {
  try {
    ...
    const prompt = `Ти — консультант з орг-структури. За канонічним шаблоном і НАЯВНИМИ теками компанії "${c.name}"${c.abbr ? ` (префікс ${c.abbr})` : ''} запропонуй впорядковану структуру папок Google Drive.\nВАЖЛИВО — стислість, щоб уміститись:\n- Показуй структуру ДО РІВНЯ ПОСАД включно. НЕ розгортай стандартні підпапки кожної посади (Посадові інструкції, Доступні документи, Доступи, Звітність) — вони однакові скрізь; згадай це ОДИН раз у descUser відповідного відділення.\n- descUser: 1 стисле речення (що зберігати). descSystem: 3-6 ключових слів для авто-маршрутизації файлів.\nДе можливо — зістав наявні теки з шаблоном. Поверни РІВНО один JSON без тексту навколо:\n{"structure":[{"name":"...","type":"folder","descUser":"...","descSystem":"...","children":[...]}]}\nУкраїнською.\n\nШАБЛОН:\n${STRUCTURE_TEMPLATE}\n\nНАЯВНІ ТЕКИ:\n${current}`;
